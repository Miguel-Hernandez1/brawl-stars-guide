...
func (b *BattleIngestor) IngestBattle(ctx context.Context, entry apiclient.BattleEntry) (IngestResult, error) {
	...
	// Collect all participant tags for fingerprinting.
	allTags := collectParticipantTags(entry.Battle.Teams)
	if len(allTags) == 0 {
		return IngestResult{}, fmt.Errorf("battle at %s has no participants", entry.BattleTime)
	}
			...
			if normalTag != b.discovererTag {
				if ok, err := b.shouldEnqueue(ctx, normalTag); err != nil {
					...
					if err := queries.EnqueueDiscoveredPlayer(ctx, b.pool,
						...
						newDiscoveries = append(newDiscoveries, normalTag)
					}
				}
			}
		}
...
// collectParticipantTags flattens all teams into a single list of raw player tags.
func collectParticipantTags(teams [][]apiclient.BattleParticipant) []string {
	var tags []string
